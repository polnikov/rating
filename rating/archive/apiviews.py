...
class ArchiveApiView(LoginRequiredMixin, View):
   def get(self, request):
      ...
      students = Student.objects.select_related().filter(**filter)
      results = Result.objects.select_related().filter(**filter)
      subjects = Subject.objects.select_related().filter(**filter)
      group_subjects = GroupSubject.objects.select_related().filter(**filter)
      groups = Group.objects.select_related().filter(**filter)
